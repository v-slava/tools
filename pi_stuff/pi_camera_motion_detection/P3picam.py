...
diskSpaceToReserve = 500 * 1024 * 1024 # Keep 40 mb free on disk
...
motionState = False
settings={}
settings["PicturePath"] = "/home/pi/cam/media/"
settings["ImageName"] = "%Y.%m.%d-%H%M%S"



def captureVideo(filename, path):
    # Generate the picture's name
    picName = path + filename + '.h264'
    with picamera.PiCamera() as camera:
        # Start recording
        camera.start_recording(picName)
        for x in range(0, 60):
            print ("wait %d" % (x))
            time.sleep(1)
        # Stop recording
        camera.stop_recording()
    print("We have taken a movie.")
    return picName
...
def saveImage(filename, filepath):
    subprocess.call("raspistill -t 200 -e jpg -o %s%s.jpg" % (filepath,filename), shell=True)
    print("Captured %s" % filename)

# Keep free space above given level
def keepDiskSpaceFree(bytesToReserve):
    if (getFreeSpace() < bytesToReserve):
        for filename in sorted(os.listdir(settings["PicturePath"])):
            if (filename.endswith(".h264") or filename.endswith(".jpg")):
                os.remove(settings["PicturePath"] + filename)
                print("Deleted %s%s to avoid filling disk" % (settings["PicturePath"],filename))
                if (getFreeSpace() > bytesToReserve):
                    return

# Get available disk space
def getFreeSpace():
    st = os.statvfs(settings["PicturePath"])
...
def motion():
    ...
    while (True):
        ...
        if (debugMode):
            debugimage.save(settings["PicturePath"] + "debug.bmp") # save debug image as bmp
...
while True:
    ...
    if motionState:
        currentTime = getTime()
        picName = currentTime.strftime(settings["ImageName"]) 
        saveImage(picName, settings["PicturePath"])        
        captureVideo(picName, settings["PicturePath"])